...
if __name__ == '__main__':
    ...
    reg_mf = 0
    num_negatives = int(neg)  #number of negatives per positive instance
    ...
    log_name="CMemNN_no_cross(%s) with %s Dropout %s: mf_dim=%d, layers=%d, regs=%f, reg_mf=%.1e, num_negatives=%d, weight_negatives=%.2f, learning_rate=%.1e, num_epochs=%d, batch_size=%d, verbose=%d, upass=%d, ipass=%d"%(dataset_name, str(sims), enable_dropout, mf_dim, layers, reg_layers, reg_mf, num_negatives, weight_negatives, learning_rate, num_epochs, batch_size, verbose, upass, ipass)
    logging_mark=1
